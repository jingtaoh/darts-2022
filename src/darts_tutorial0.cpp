...
void test_vectors_and_matrices()
{
    ...
    float degrees = Spherical::rad2deg(acos(dot(normalize(v1), normalize(v3))));
...
void test_color_and_image()
{
    ...
    // Store the reciprocals
    float inv_h = 1.0f / gradient.height();
    float inv_w = 1.0f / gradient.width();
    ...
    for (int row = 0; row < gradient.height(); row++)
    {
        ...
        {
            gradient(col, row) = Color3f(lerp(0, 1, col * inv_w), // R
                                         lerp(0, 1, row * inv_h), // G
                                         0);                      // B
        }
    }
    ...
    for (int row = 0; row < image.height(); row++)
    {
        ...
        {
            image(col, row) = Color3f(luminance(image(col, row)));
        }
    }
